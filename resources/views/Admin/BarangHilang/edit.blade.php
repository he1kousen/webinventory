    ...
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div class="modal-content modal-content-demo">
            <div class="modal-header">
                <h6 class="modal-title">Ubah Barang Hilang</h6><button aria-label="Close" onclick="resetU()" class="btn-close" data-bs-dismiss="modal"><span aria-hidden="true">&times;</span></button>
            </div>
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-6">
                        <input type="hidden" name="idbhU">
                        <div class="form-group">
                            <label for="bhkodeU" class="form-label">Kode Barang Hilang <span class="text-danger">*</span></label>
                            <input type="text" name="bhkodeU" readonly class="form-control" placeholder="">
                        </div>
                        <div class="form-group">
                            <label for="tglhilangU" class="form-label">Tanggal Hilang <span class="text-danger">*</span></label>
                            <input type="text" name="tglhilangU" class="form-control datepicker-date" placeholder="">
                        </div>
                        <div class="form-group">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            <label for="jmlU" class="form-label">Jumlah Hilang <span class="text-danger">*</span></label>
                            ...
                            <input type="hidden" name="jmlLamaU" value="">
                        </div>
                    </div>
                </div>
...
<script>
    ...
    function checkFormU() {
        const tglhilang = $("input[name='tglhilangU']").val();
        ...
        const jmlLama = parseInt($("input[name='jmlLamaU']").val() || 0);
        setLoadingU(true);
        resetValidU();

        if (tglhilang == "") {
            validasi('Tanggal Hilang wajib di isi!', 'warning');
            $("input[name='tglhilangU']").addClass('is-invalid');
            setLoadingU(false);
            return false;
            ...
            $.ajax({
                type: 'GET',
                url: '/admin/barang-hilang/cek-stok/' + kdbarang,
                dataType: 'json',
                success: function(response) {
                    var stokTersedia = parseInt(response.stok) + jmlLama;
                    if (response.stok !== undefined && parseInt(jml) > stokTersedia) {
                        validasi('Jumlah hilang melebihi stok yang tersedia!', 'warning');
                        $("input[name='jmlU']").addClass('is-invalid');
                        setLoadingU(false);
                        return false;
                    } else {
    ...
    function submitFormU() {
        const id = $("input[name='idbhU']").val();
        const bhkode = $("input[name='bhkodeU']").val();
        const tglhilang = $("input[name='tglhilangU']").val();
        const kdbarang = $("input[name='kdbarangU']").val();
        const tujuan = $("input[name='tujuanU']").val();
        const jml = $("input[name='jmlU']").val();
        ...
        $.ajax({
            type: 'POST',
            url: "{{ url('admin/barang-hilang/proses_ubah') }}/" + id,
    ...
    function resetValidU() {
        $("input[name='tglhilangU']").removeClass('is-invalid');
    ...
    function resetU() {
        resetValidU();
        $("input[name='idbhU']").val('');
        $("input[name='bhkodeU']").val('');
        $("input[name='tglhilangU']").val('');
        $("input[name='kdbarangU']").val('');
        $("input[name='tujuanU']").val('');
        $("input[name='jmlU']").val('0');
        $("input[name='jmlLamaU']").val('');
        $("#nmbarangU").val('');
        $("#satuanU").val('');
        $("#jenisU").val('');
        $("#statusU").val('false');

    ...
    public function show(Request $request)
    {
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('jmlrusak', function ($row) use ($request) {
                    if ($request->tglawal) {
                        $jmlrusak = BarangrusakModel::leftJoin('tbl_barang', 'tbl_barang.barang_kode', '=', 'tbl_barangrusak.barang_kode')->whereBetween('br_tanggal', [$request->tglawal, $request->tglakhir])->where('tbl_barangrusak.barang_kode', '=', $row->barang_kode)->sum('tbl_barangrusak.br_jumlah');
                    } else {
                        $jmlrusak = BarangrusakModel::leftJoin('tbl_barang', 'tbl_barang.barang_kode', '=', 'tbl_barangrusak.barang_kode')->where('tbl_barangrusak.barang_kode', '=', $row->barang_kode)->sum('tbl_barangrusak.br_jumlah');
                    }

                    $result = '<span class="">'.$jmlrusak.'</span>';

                    return $result;
                })
                ->addColumn('jmlhilang', function ($row) use ($request) {
                    if ($request->tglawal) {
                        $jmlhilang = BaranghilangModel::leftJoin('tbl_barang', 'tbl_barang.barang_kode', '=', 'tbl_baranghilang.barang_kode')->whereBetween('bh_tanggal', [$request->tglawal, $request->tglakhir])->where('tbl_baranghilang.barang_kode', '=', $row->barang_kode)->sum('tbl_baranghilang.bh_jumlah');
                    } else {
                        $jmlhilang = BaranghilangModel::leftJoin('tbl_barang', 'tbl_barang.barang_kode', '=', 'tbl_baranghilang.barang_kode')->where('tbl_baranghilang.barang_kode', '=', $row->barang_kode)->sum('tbl_baranghilang.bh_jumlah');
                    }

                    $result = '<span class="">'.$jmlhilang.'</span>';
                ...
                ->addColumn('totalstok', function ($row) use ($request) {
                    ...
                    $totalstok = $row->barang_stok + ($jmlmasuk - $jmlkeluar - $jmlrusak - $jmlhilang);
                ...
                ->rawColumns(['stokawal', 'jmlmasuk', 'jmlkeluar', 'jmlrusak', 'jmlhilang', 'totalstok'])->make(true);
        }
    }
}
